...
function gsylv_group(problem)
    ...
    g["enzyme_batch_forward"] = @benchmarkable Enzyme.autodiff(
        ...
        D = copy($(problem.D))
        E = copy($(problem.E))
        A_tans = ntuple(i -> copy($(problem.dA_lanes)[i]), Val($GSYLV_ENZ_LANES))
        B_tans = ntuple(i -> copy($(problem.dB_lanes)[i]), Val($GSYLV_ENZ_LANES))
        C_tans = ntuple(i -> copy($(problem.dC_lanes)[i]), Val($GSYLV_ENZ_LANES))
        D_tans = ntuple(i -> copy($(problem.dD_lanes)[i]), Val($GSYLV_ENZ_LANES))
        E_tans = ntuple(i -> copy($(problem.dE_lanes)[i]), Val($GSYLV_ENZ_LANES))
        W = $(problem.W)
    end evals = 1

    g["forwarddiff_jvp"] = @benchmarkable gsylv_loss(A_dual, B_dual, C_dual, D_dual, E_dual, W) setup = begin
...
g = BenchmarkGroup()
g["small"]  = gsylv_group(gsylv_small_problem())
g["medium"] = gsylv_group(gsylv_medium_problem())
g
