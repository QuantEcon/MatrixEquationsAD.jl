...
@testset "out-of-place ordqz wrappers" begin
    ...
    @testset "Enzyme heap reverse" begin
        dA = zero(A); dB = zero(B)
        autodiff(Reverse, _oop_ordqz_loss, Active, Duplicated(A, dA), Duplicated(B, dB))
        fd = jvp(
            _oop_fdm, (Av, Bv) -> _oop_ordqz_loss(Av, Bv),
            (A, dA), (B, dB),
        )
    ...
    @testset "Enzyme SMatrix paths match heap" begin
        ...
        fwd_h = only(
            autodiff(
                Forward, _oop_ordqz_loss, Duplicated(A, dA_dir), Duplicated(B, dB_dir),
            )
        )
        fwd_s = only(
            autodiff(
                Forward, _oop_ordqz_loss, Duplicated(As, dAs), Duplicated(Bs, dBs),
            )
        )
        @test fwd_h ≈ fwd_s
    end
    ...
    @testset "ForwardDiff SMatrix path" begin
        ...
        ro = ordqz(As_dual, Bs_dual, :bk; threshold = 1.0e-6)
        @test ro.S isa SMatrix{3, 3}
        ro_v = ordqz(As, Bs, :bk; threshold = 1.0e-6)
        @test all(ForwardDiff.value.(ro.S) .≈ ro_v.S)
    end
end
