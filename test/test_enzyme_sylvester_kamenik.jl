...
# Test tier flag — see test/runtests.jl. Re-read here so the file can be
# `include`d standalone (e.g. iterating in the REPL).
const _RUN_SLOW_TESTS = get(ENV, "RUN_SLOW_TESTS", "false") == "true"

# Primal fixtures: always include RBC/RBC_SV/SGU; FVGQ (n=38, exercises
# the 2×2 complex Schur block path) only under RUN_SLOW_TESTS to keep
# default CI fast.
const FIXTURES = let base = (
        ("rbc", rbc_second_order_sylvester_inputs),
        ("rbc_sv", rbc_sv_second_order_sylvester_inputs),
        ("sgu", sgu_second_order_sylvester_inputs),
    )
    _RUN_SLOW_TESTS ? (base..., ("fvgq", fvgq_second_order_sylvester_inputs)) : base
end
...
if _RUN_SLOW_TESTS
    # ~3.5 min on cold CI: full EnzymeTestUtils sweep on the allocating
    # form (forward + reverse, Const-arg branches). The manual FD-vs-Enzyme
    # tests further down already cross-check the math, so this sweep is
    # primarily for catching Enzyme version regressions.
    ...
    # ~1.5 min — same sweep on the in-place form.
    @testset "gsylv_kamenik! Enzyme test_forward / test_reverse — RBC" begin
        (; A, B, C, D) = rbc_second_order_sylvester_inputs()
...
if _RUN_SLOW_TESTS
    ...
    @testset "gsylv_kamenik Enzyme reverse smoke — large fixtures" begin
        for (name, builder) in smoke_fixtures
            (; A, B, C, D) = builder()
            A_sh = zero(A); B_sh = zero(B); C_sh = zero(C); D_sh = zero(D)
            autodiff(
                Reverse, kamenik_loss, Active,
    ...
    # ~1.5 min each — BatchDuplicated(4) forward consistency at SGU
    # scale. Catches batched-rule regressions; default CI relies on the
    # SGU reverse smoke above + the RBC EnzymeTestUtils sweep.
